...
const ExpenseEntryForm = ({ onExpenseAdded, categories, onCategoryCreate }) => {
  ...
  const generateStoryImpact = (data) => {
    ...
    if (amount > 100) {
      ...
      } represents a significant chapter in your financial story, potentially affecting your monthly budget by ${(
        (amount / 2000) *
        100
      )?.toFixed(1)}%.`;
    } else {
      return `A small but meaningful addition to your ${
        category?.name || "spending"
      } story, contributing to your daily financial narrative.`;
    }
  };
