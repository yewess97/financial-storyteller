...
const BudgetImpactIndicator = ({
  ...
  const getStoryNarrative = () => {
    if (impact?.isOverBudget) {
      ...
      return `This expense would take you $${overAmount?.toFixed(
        2
      )} over your budget, adding an unexpected twist to this month's financial story.`;
    }

    if (impact?.percentageUsed > 90) {
      return `You're approaching the climax of this category's budget story - only $${impact?.remainingBudget?.toFixed(
        2
      )} remaining.`;
    }

    if (impact?.percentageUsed > 70) {
      return `This expense moves your story into the rising action phase, with $${impact?.remainingBudget?.toFixed(
        2
      )} left for this category.`;
    }

    return `A comfortable addition to your financial narrative, leaving $${impact?.remainingBudget?.toFixed(
      2
    )} for future chapters in this category.`;
  };
    ...
    <div className="bg-card border border-border rounded-lg p-4 shadow-warm-sm">
      <div className="flex items-start space-x-3">
        <div
          className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
            impact?.isOverBudget
              ? "bg-error/10"
              : impact?.percentageUsed > 80
              ? "bg-warning/10"
              : "bg-success/10"
          }`}
        >
          <Icon name={getImpactIcon()} size={16} className={getImpactColor()} />
        </div>
      ...
      <div className="mt-4 p-3 bg-muted/30 rounded-lg">
        <div className="flex items-start space-x-2">
          ...
          />
          <p className="text-sm text-muted-foreground">{getStoryNarrative()}</p>
        </div>
      </div>
      {impact?.isOverBudget && (
          ...
          <div className="flex items-start space-x-2">
            ...
            <div className="text-sm">
              ...
              <p className="text-muted-foreground">
                Consider adjusting this expense or reallocating from another
                category to keep your financial story on track.
              </p>
            </div>
          </div>
        </div>
